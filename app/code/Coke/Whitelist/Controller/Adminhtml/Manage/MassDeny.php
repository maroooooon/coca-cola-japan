...
{
    const ADMIN_RESOURCE = 'Coke_Whitelist::whitelist_deny';
    ...
    public function execute()
    {
        try {
            ...
            foreach ($collection->getItems() as $whitelist) {
                $result = $this->whitelistService->deny($whitelist->getId());

                if (!$result) {
                    throw new LocalizedException(__('Something went wrong trying to deny %1.', $whitelist->getValue()));
                }
            }
