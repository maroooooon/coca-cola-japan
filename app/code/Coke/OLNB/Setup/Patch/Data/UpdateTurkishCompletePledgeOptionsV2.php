...
class UpdateTurkishCompletePledgeOptionsV2 implements DataPatchInterface
{
    /**
     * @var LoggerInterface
     */
    ...
    {
        $this->updateTitleAndSortOrder('Complete your Pledge', 'Seçtiğin söz');
    ...
    private function getCustomProductOptionData(int $storeId, string $title): array
    {
        ...
        $select = $connection->select()->from(
            ...
            "cpo.step_label = 'Complete your Pledge'"
        )->where(
            "cpot.title = ?", $title
        )->distinct(true);
    ...
    {
        $storeId = $this->getStoreIdByCode('turkey_turkish');
