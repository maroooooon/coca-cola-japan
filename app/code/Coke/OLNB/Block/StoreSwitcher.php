...
class StoreSwitcher extends \Magento\Framework\View\Element\Template
{
    ...
    public function getRawGroups()
    {
        if (!$this->hasData('raw_groups')) {
            $websiteGroups = $this->groupCollectionFactory->create()
                ->addFieldToFilter('code', ['like' => 'olnb%'])
                ->getItems();
    ...
    public function getRawStores()
    {
        if (!$this->hasData('raw_stores')) {
            $groups = $this->getRawGroups();
            $groupIds = array_map(function ($g) {
                return $g['group_id'];
            }, $groups);
            $websiteStores = $this->storeCollectionFactory->create()
